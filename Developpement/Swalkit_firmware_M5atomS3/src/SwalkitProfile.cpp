...
void SwalkitProfile::fromBytes(const uint8_t *data) {
    size_t name_length = data[0];
    name = std::string(reinterpret_cast<const char *>(data + 1), name_length);
    const uint8_t *pData = data + name_length + 1;
...
const uint8_t* SwalkitProfile::toBytes(size_t &data_length) {
    size_t name_length = std::min(name.size(), BufferSize - 13);
    data_length = name_length + 13;
    dataBuffer[0] = static_cast<uint8_t>(name_length);
